...
EXP_dis_MLE <- function(x,xmin){
  return( log( length(x) / sum(x-xmin) + 1) )
}
...
EXP_dis_llh <- function(x,xmin,a){
  return( length(x)*log(1-e^a) + length(x)*a*xmin - a*sum(x)  )
}
...
ID = ID2 = sep_time = tan_time = cens = cens2 = sep_male_speed = sep_female_speed = sep_dis <- NULL
...
ge.sf<-survfit(Surv(sep_time,cens2)~0)
par(mfrow=c(1,1), pin=c(4,3))
plot(ge.sf, xlab="time (sec)", ylab="probability of tandem running", main="R. speratus", las=1)
Rs_septime <- sep_time
Rs_cens2 <- cens2
...
lambda.est_P <- EXP_dis_MLE(Stop_sec, min_sec)
...
lambda.est <- EXP_dis_MLE(Move_sec, min_sec)
lambda.est # 1.261352
