...
plot(Stop_lab,Stop_cum, log="xy", main=paste(Treat,Sex))
...
idea.x <- seq(0,0.99999,1/length(Stop_sec))
y1.est <- PL(idea.x, myu, min_sec)
y2.est <- EXP(idea.x, lambda.est, min_sec)
...
###################
## 170923
## 上に対して、pause/moveのデータをrandomizeして、surrogate dataを作成
## このsurrogate dataに対してfittingを行う

setwd("E:\\Dropbox\\research\\with_dropbox\\papers and projects\\2017\\Sex movement after separation\\Rs\\solo-tandem\\location_data\\5FPS\\170623")
Folder.path <- getwd()
f.namesplace <- list.files(Folder.path, pattern=".txt",full.names=T)

# 1. 各個体別々にrandomizeした後、poolしてfitting
...
for(j in 1:length(f.namesplace)){
  ...
  L <- min3+count
  sur_stop <- sample(stop[1:L], L, replace=randomize_replace)
...
# 2 rank plot
rank <- seq(length(Stop_sec),1,-1)
plot(sort(Stop_sec),rank,log="xy",xlab="value",ylab="rank", main=paste(Treat,Sex))
...
# 2. 全データプールしてrandomizeした後、fitting
randomize_replace <- FALSE
...
for(j in 1:length(f.namesplace)){
  ...
  L <- min3+count
  Stop <- c(Stop,stop[2:L])
}

sur_stop <- sample(Stop, length(Stop), replace=randomize_replace)
