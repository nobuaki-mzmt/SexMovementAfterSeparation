...
Colony = Sex = ID = Minutes = Treat = Mu = Rho = Moved = Stop_time = True_speed <- NULL
for(k in 1:length(before)){
  for(j in c(1,3)){
    ...
    if(j==1){ anal_time <-  (60*27):(60*30)+1}
    if(j==3){ anal_time <-  (60*0):(60*3)+1}
...
d <- data.frame(sex=Sex, treat=Treat, moved=Moved, stop=Stop_time, speed=True_speed, Rho=Rho, Mu=Mu)
...
# moved distance
Mean <- (tapply(d[,3],d[,1:2],mean))[2:1,2:1]
SE <- (tapply(d[,3],d[,1:2],se))[2:1,2:1]

Fig <- barplot(Mean, col=c(1,0), pch=19, ylim=c(0,650), beside=T, las=1,
...
# stop time
Mean <- (tapply(d[,4],d[,1:2],mean))[2:1,2:1]
SE <- (tapply(d[,4],d[,1:2],se))[2:1,2:1]

Fig <- barplot(Mean, col=c(1,0), pch=19, ylim=c(0,30), beside=T, las=1,
               ylab="stop time (sec)")
arrows(Fig , Mean, Fig , Mean + SE, angle=90,length=0.1)
legend("top", c("male","female"), fill=c(1,0))　

wilcox.exact(stop ~ sex, data=d[d$treat=="after",], paired=F)
wilcox.exact(stop ~ sex, data=d[d$treat=="before",], paired=F)
wilcox.exact(stop ~ treat, data=d[d$sex==0,], paired=T)
wilcox.exact(stop ~ treat, data=d[d$sex==1,], paired=T)


# True speed
Mean <- (tapply(d[,5],d[,1:2],mean))[2:1,2:1]
SE <- (tapply(d[,5],d[,1:2],se))[2:1,2:1]
...
wilcox.exact(speed ~ sex, data=d[d$treat=="after",], paired=F)
wilcox.exact(speed ~ sex, data=d[d$treat=="before",], paired=F)
wilcox.exact(speed ~ treat, data=d[d$sex==0,], paired=T)
wilcox.exact(speed ~ treat, data=d[d$sex==1,], paired=T)


# Rho (wrapped caucy distribution)
Mean <- (tapply(d[,6],d[,1:2],mean))[2:1,2:1]
SE <- (tapply(d[,6],d[,1:2],se))[2:1,2:1]
...
wilcox.exact(Rho ~ treat, data=d[d$sex==0,], paired=T)
