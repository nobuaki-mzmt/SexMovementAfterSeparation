...
for(j in 1:length(f.namesplace)){
  ...
  ## tandemは距離7mm以下になるとtandem再開とする
  ## 距離6mm以下が5sec以上続いて、初めてタンデムとみなす
  L <- length(time)
  separate <- MF_dis > 10
  tandem <- MF_dis < 10
  tan_beg <- 0
 ...
 # plot(time, MF_dis, col=sep_label, main=paste(id, j))
  #plot(time[time>1700 & time<2000], MF_dis[time>1700 & time<2000], col=sep_label[time>1700 & time<2000])
}
...
res2 <- res[res[,3]<15 & res[,2]<100 & res[,2]>2,]
res2 <- res[res[,2]>2,]
plot(res2[,3], res2[,2])
cor.test(res2[,3], res2[,2])
