...
int initial(int id, int pattern) {
	...
	case 1:
		...
		break;
	case 2:
		if (Rn <= 0.5) { Pnokori[id] = rexp(0.1217); Wnokori[id] = 0; }
		else { Pnokori[id] = 0; Wnokori[id] = rexp(1.2614); }
		break;
	case 0:
	case 3:
	case 4:
...
int main() {
	...
	cout << "Default: 16.09702 mm, OK?" << endl;
	cin >> OK;
	if (OK != 1) {
