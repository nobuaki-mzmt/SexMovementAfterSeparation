...
int initial(int id, int pattern, int emp, int p) {
	Rn = rnd();
	Angle[id] = allocate();
	if (emp != 1) {
		if (p == 1 && pattern == 1) {
			Pnokori[id] = 100000; Wnokori[id] = 0;
		}
		else if (p == 2 && pattern == 1) {
			Wnokori[id] = 100000; Pnokori[id] = 0;
		}
		else {
...
double Sep_search(vector<int>& MovementPattern, int& iter, int& steps, vector<int>& res, double& d, int emp, int p) {
	for (i = 0; i < iter; i++) {
		...
		for (j = 0; j < 2; j++) {
			initial(j, MovementPattern[j], emp, p);
		}
...
double Ran_search(vector<int>& MovementPattern, int& iter, int& steps, vector<int>& res, double& L, int emp, int p) {
	for (i = 0; i < iter; i++) {
		printf("female:%d, male:%d, rep:%d        \r", MovementPattern[0], MovementPattern[1], i + 1);
		// initialization
		LocX[0] = 0;
...
int main() {
	...
	int MaleMovementPattern[3] = { 3,3,3 };	// movement patterns for males
	int FemaleMovementPattern[3] = { 1,1,1 };	// movement patterns for females
	int p;
	for (p = 0; p < 3; p++) {
		// run simulations
		...
		if (search_mode == 0) {
			...
			Ran_search(MovementPattern, iter, steps, res, L, emp, p);
		}
			...
			Sep_search(MovementPattern, iter, steps, res, d, emp, p);
		}
